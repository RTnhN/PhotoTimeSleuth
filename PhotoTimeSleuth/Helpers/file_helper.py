    ...
    with open(bday_file, "r") as f:
        for line in f:
            ...
            if year < 1900 or year > 2100:
                raise FormatError(
                    f"Invalid year in bday file. Make sure the year ({year}) is between 1900 and 2100. Also make sure that the order is correct (e.g. YYYY-MM-DD)."
                )
            if month < 1 or month > 12:
                raise FormatError(
                    f"Invalid month in bday file. Make sure the month ({month}) is between 1 and 12. Also make sure that the order is correct (e.g. YYYY-MM-DD)."
                )
            if day < 1 or day > 31:
                raise FormatError(
                    f"Invalid day in bday file. Make sure the day({day}) is between 1 and 31. Also make sure that the order is correct (e.g. YYYY-MM-DD)."
                )
